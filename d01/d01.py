...
def count_number_of_times_depth_measurement_increases(measurements):
    return sum([x[1] > x[0] for x in zip(measurements[0:-1], measurements[1:])])
...
if __name__ == '__main__':
    ...
    three_sliding_window_measurements = list(map(sum, zip(measurements[0:-2], measurements[1:-1], measurements[2:])))
